...
SSDP.prototype._start = function (port, host, cb) {
  ...
  this.sock.on('listening', function onSocketListening() {
    ...
    var addMembershipTries = 0;
    function addMembership() {
      addMembershipTries++;
      if (addMembershipTries >= 10) {
        self._started = false;
        cb && cb(new Error('Too many addMembership attempts.'));
        return;
      }
      try {
        ...
        if (e.code === 'ENODEV') {
          ...
          setTimeout(addMembership, 1000)
        } else if (e.code === 'ENOMEM') {
          self._logger.warn(e, 'Not enough free memory to add multicast group membership. Scheduling a retry.');
          setTimeout(addMembership, 1000)
        } else {
          self._started = false;
          cb && cb(e);
  ...
  this.sock.bind(port, host, function () {
    self._logger.info('Socket bound');
  });
}
...
SSDP.prototype.setUSN = function (usn) {
  this._usns = {};
  this._usns[this._udn] = usn;
}
